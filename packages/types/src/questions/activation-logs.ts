...
export interface TriggerActivationLog {
  ...
  call_position: number; // Order in call sequence
  was_asked: boolean;
  skip_reason?: string; // Present when was_asked=false
}
...
export interface ActivationLogAnalytics {
  ...
  call_efficiency: {
    ...
    utilization_rate: number; // actual/planned
  };
...
export interface BusinessActivationReport {
  ...
  trigger_performance: Array<{
    ...
    success_rate: number; // was_asked / activation_count
  ...
  question_performance: Array<{
    ...
    activation_count: number;
    ask_rate: number; // times_asked / activation_count
    avg_call_position: number;
    skip_reasons: Array<{
      reason: string;
      count: number;
