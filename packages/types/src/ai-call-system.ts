// AI Call System Types - Step 5.1 GPT-4o-mini Integration
...
// Core AI Call Session
export interface FeedbackCallSession {
  id: string; // UUID
  customer_verification_id: string; // UUID foreign key
  store_id: string; // UUID foreign key
  phone_number: string; // Encrypted
  session_status: SessionStatus;
  call_initiated_at: string; // ISO timestamp
  call_connected_at: string | null; // ISO timestamp
  call_ended_at: string | null; // ISO timestamp
  duration_seconds: number | null;
  retry_count: number; // Default 0, max 3
  failure_reason: string | null;
  openai_session_id: string | null;
  created_at: string; // ISO timestamp
  expires_at: string; // ISO timestamp (90 days)
}

// Conversation Transcript
export interface ConversationTranscript {
  id: string; // UUID
  call_session_id: string; // UUID foreign key
  speaker: Speaker;
  message_order: number;
  content: string; // Encrypted
  timestamp_ms: number; // Milliseconds from call start
  confidence_score: number | null; // 0.00-1.00
  language_detected: string; // Default 'sv'
...
export interface QualityAssessment {
  id: string; // UUID
  call_session_id: string; // UUID foreign key
  legitimacy_score: number; // 0.00-1.00
  depth_score: number; // 0.00-1.00
  usefulness_score: number; // 0.00-1.00
  overall_quality_score: number; // 0.00-1.00
  reward_percentage: number; // 2.00-15.00
  is_fraudulent: boolean; // Default false
  fraud_reasons: Record<string, any> | null; // JSONB
  analysis_summary: string | null; // Encrypted
  business_actionable_items: Record<string, any> | null; // JSONB
  analysis_metadata: Record<string, any> | null; // JSONB
  created_at: string; // ISO timestamp
}
...
export interface BusinessContextProfile {
  id: string; // UUID
  store_id: string; // UUID foreign key
  context_version: number; // Default 1
  operating_hours: Record<string, any>; // JSONB
  departments: Record<string, any>; // JSONB
  current_campaigns: Record<string, any> | null; // JSONB
  question_configuration: Record<string, any>; // JSONB
  baseline_facts: Record<string, any>; // JSONB
  context_completeness_score: number; // 0.00-1.00, default 0.50
...
export interface WeeklyAnalysisReport {
  id: string; // UUID
  store_id: string; // UUID foreign key
  analysis_week: string; // Date (Monday of analysis week)
  total_feedback_count: number;
  average_quality_score: number | null;
  positive_trends: Record<string, any> | null; // JSONB
  negative_issues: Record<string, any> | null; // JSONB
  new_issues: Record<string, any> | null; // JSONB
  department_insights: Record<string, any> | null; // JSONB
  historical_comparison: Record<string, any> | null; // JSONB
  predictive_insights: Record<string, any> | null; // JSONB
  actionable_recommendations: Record<string, any> | null; // JSONB
  report_metadata: Record<string, any> | null; // JSONB
...
export interface CallQualityMetrics {
  ...
  audio_clarity_score: number | null; // 0.00-1.00
  latency_ms: number | null;
  packet_loss_percentage: number | null;
  openai_api_latency: number | null;
  technical_errors: Record<string, any> | null; // JSONB
...
export interface FraudDetectionResult {
  ...
  fraud_indicators: Record<string, any> | null; // JSONB
  context_violations: Record<string, any> | null; // JSONB
...
export interface CleanupLowGradeRequest {
  quality_threshold: number; // Minimum score to retain (0.02 = 2%)
  older_than_days?: number; // Default 90 days
  store_ids?: string[]; // Optional store filter
}
...
export interface GenerateReportRequest {
  store_id: string;
  analysis_week: string; // YYYY-MM-DD (Monday)
...
// Database Table Types for AI System
