...
exports.CustomerRewardBatchModel = void 0;
const supabase_1 = require("../client/supabase");
class CustomerRewardBatchModel {
     ...
     * Update batch status
     */
    static async updateStatus(id, status, deliveryDetails) {
        const updateData = {
            status,
            updated_at: new Date().toISOString()
        };
     ...
     */
    static async updateExportFiles(id, exportFiles) {
        const { data: batch, error } = await this.getSupabaseClient()
            .from('customer_reward_batches')
            .update({
     ...
     */
    static async getBatchStatistics(cycleId) {
     ...
     */
    static async bulkCreate(batches) {
     ...
     */
    static async getBatchesReadyForDelivery() {
        const { data: batches, error } = await this.getSupabaseClient()
            .from('customer_reward_batches')
            .select('*')
            .eq('status', 'generated')
     ...
     */
    static async updateDeliveryDetails(id, deliveryDetails) {
        const { data: batch, error } = await this.getSupabaseClient()
            .from('customer_reward_batches')
            .update({
     ...
     */
    static async getByExportFormat(format, options = {}) {
        let query = supabase
            .from('customer_reward_batches')
            .select('*')
...
exports.CustomerRewardBatchModel = CustomerRewardBatchModel;
//# sourceMappingURL=customer-reward-batches.js.map