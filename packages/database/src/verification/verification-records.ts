...
export class VerificationRecordModel {
   ...
   */
  static async getById(id: string): Promise<VerificationRecord | null> {
    const { data: record, error } = await this.getSupabaseClient()
      .from('verification_records')
      .select('*')
      .eq('id', id)
   ...
   */
  static async getStatistics(databaseId: string): Promise<{
    total: number;
    pending: number;
    verified: number;
    rejected: number;
   ...
   */
  static async getByPhoneNumber(phoneNumber: string): Promise<VerificationRecord[]> {
    const { data: records, error } = await this.getSupabaseClient()
      .from('verification_records')
      .select('*')
   ...
   */
  static async getExpiredRecords(cutoffDate: Date): Promise<VerificationRecord[]> {
    const { data: records, error } = await this.getSupabaseClient()
      .from('verification_records')
      .select('*')
      .eq('status', 'pending')
