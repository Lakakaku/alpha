...
exports.ContextAnalysisModel = void 0;
const supabase_1 = require("../client/supabase");
class ContextAnalysisModel {
    static TABLE_NAME = 'context_analyses';
    /**
     ...
     */
    static async getLatestByPhoneHash(phoneHash) {
        const { data, error } = await supabase_1.supabase
            .from(this.TABLE_NAME)
            .select('*')
            .eq('phone_hash', phoneHash)
     ...
     */
    static async getByLanguage(language, options) {
        let query = supabase_1.supabase
            .from(this.TABLE_NAME)
            .select('*')
     ...
     */
    static async searchByContent(searchQuery, options) {
        let query = supabase_1.supabase
            .from(this.TABLE_NAME)
            .select('*')
    ...
    static async getStatistics(options = {}) {
        ...
        const impossibleClaimsCount = data.filter(analysis => analysis.gpt_analysis?.impossible_claims_detected && analysis.gpt_analysis.impossible_claims_detected.length > 0).length;
        const suspiciousPatternsCount = data.filter(analysis => analysis.gpt_analysis?.suspicious_patterns && analysis.gpt_analysis.suspicious_patterns.length > 0).length;
        // Average processing time
        const averageProcessingTime = data.reduce((sum, analysis) => sum + (analysis.gpt_analysis?.processing_time_ms || 0), 0) / totalAnalyses;
        // Score distribution
     ...
     */
    static async getBulkLatest(phoneHashes) {
...
exports.ContextAnalysisModel = ContextAnalysisModel;
