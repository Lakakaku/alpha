...
exports.BehavioralPatternModel = void 0;
const supabase_1 = require("../client/supabase");
class BehavioralPatternModel {
    static TABLE_NAME = 'behavioral_patterns';
    /**
     ...
     */
    static async getByPhoneHash(phoneHash, options) {
     ...
     */
    static async updatePattern(id, updates) {
        const updateData = {
            last_updated: new Date().toISOString()
        };
     ...
     */
    static async getCallFrequencyPatterns(phoneHash, timeWindowMinutes = 30) {
     ...
     */
    static async getSimilarityPatterns(phoneHash) {
        const { data, error } = await supabase_1.supabase
            .from(this.TABLE_NAME)
            .select('*')
            .eq('phone_hash', phoneHash)
     ...
     */
    static async getLocationPatterns(phoneHash) {
        const { data, error } = await supabase_1.supabase
            .from(this.TABLE_NAME)
            .select('*')
            .eq('phone_hash', phoneHash)
     ...
     */
    static async getTimePatterns(phoneHash) {
        const { data, error } = await supabase_1.supabase
            .from(this.TABLE_NAME)
            .select('*')
            .eq('phone_hash', phoneHash)
    ...
    static async getStatistics(options = {}) {
        ...
        data.forEach(pattern => {
            patternTypeDistribution[pattern.pattern_type]++;
        });
        ...
        data.forEach(pattern => {
            if (pattern.detection_rules && Array.isArray(pattern.detection_rules)) {
                pattern.detection_rules.forEach((rule) => {
                    if (rule.triggered) {
     ...
     */
    static generateResponse(phoneHash, patterns, timeWindow = '24h') {
     ...
     */
    static async resolvePattern(id, resolutionNotes) {
        const { data, error } = await supabase_1.supabase
            .from(this.TABLE_NAME)
            .update({
            is_resolved: true,
     ...
     */
    static async deleteOldResolved(daysOld) {
     ...
     */
    static async getBulkPatterns(phoneHashes, options) {
    ...
    static calculateOverallRiskLevel(patterns) {
        ...
        if (maxRiskScore >= 85 || (avgRiskScore >= 70 && totalViolations >= 5))
            return 'critical';
        if (maxRiskScore >= 70 || (avgRiskScore >= 50 && totalViolations >= 3))
            return 'high';
        if (maxRiskScore >= 40 || avgRiskScore >= 30)
            return 'medium';
        return 'low';
    }
    static generateRecommendation(riskLevel, highestRiskScore, totalViolations) {
        switch (riskLevel) {
            case 'critical':
...
exports.BehavioralPatternModel = BehavioralPatternModel;
//# sourceMappingURL=behavioral-pattern.js.map