...
export class BusinessPerformanceMetricsModel {
  /**
   * Create or update performance metrics for a store and date
   */
  static async upsertMetrics(metricsData: PerformanceData): Promise<BusinessPerformanceMetrics | null> {
  ...
  static async getMetrics(
    ...
    page = 1,
    limit = 50
  ): Promise<{ metrics: BusinessPerformanceMetrics[]; total: number }> {
    let query = supabase
      .from('business_performance_metrics')
      .select('*', { count: 'exact' });
  ...
  static async getByStoreAndDate(
    storeId: string,
    reportDate: string
  ): Promise<BusinessPerformanceMetrics | null> {
   ...
   */
  static async getStorePerformanceTrends(
    storeId: string,
    days = 30
  ): Promise<{
    trends: {
      ...
      verificationRate: number;
      customerSatisfactionScore: number;
      operationalScore: number; // aggregated operational score
    }[];
    summary: {
    ...
    const trends = (data || []).map(record => {
      ...
      const operationalMetrics = record.operational_metrics as OperationalMetrics || {};
  ...
  static async getComparativeAnalysis(
    ...
    data?.forEach(record => {
      ...
      if (record.operational_metrics) {
        storeMetrics[record.store_id].operationalMetrics.push(record.operational_metrics as OperationalMetrics);
      }
    });
    ...
    const storesWithRanks = stores.map((store, index) => {
      let performanceGrade: 'A' | 'B' | 'C' | 'D' = 'D';
      if (store.overallScore >= 90) performanceGrade = 'A';
      else if (store.overallScore >= 80) performanceGrade = 'B';
      else if (store.overallScore >= 70) performanceGrade = 'C';
  ...
  static async getPerformanceAlerts(
    ...
    const storeLatestMetrics: { [storeId: string]: {
      satisfactionScore: number;
      verificationRate: number;
      operationalScore: number;
      lastReportDate: string;
    ...
    data?.forEach(record => {
      if (!storeLatestMetrics[record.store_id] || record.report_date > storeLatestMetrics[record.store_id].lastReportDate) {
        const operationalScore = this.calculateOperationalScore(record.operational_metrics as OperationalMetrics || {});
   ...
   * Calculate operational score from multiple metrics
   */
  private static calculateOperationalScore(metrics: OperationalMetrics): number {
    ...
    const metricWeights: { [key: string]: number } = {
      uptime: 0.3,
