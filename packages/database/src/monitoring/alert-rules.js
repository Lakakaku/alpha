...
class AlertRuleModel {
     ...
     */
    static async getAlertRules(filters, page = 1, limit = 50) {
    ...
    static async update(id, updates) {
        ...
        if (updates.thresholdValue !== undefined)
            updateData.threshold_value = updates.thresholdValue;
        if (updates.comparisonOperator !== undefined)
            updateData.comparison_operator = updates.comparisonOperator;
     ...
     */
    static async getByMetricType(metricType) {
        const { data, error } = await supabase_1.supabase
            .from('alert_rules')
            .select('*')
     ...
     */
    static async getByCreator(adminId) {
        const { data, error } = await supabase_1.supabase
            .from('alert_rules')
            .select('*')
     ...
     */
    static async evaluateMetric(metricType, metricValue) {
     ...
     */
    static async createDefaultRules(adminId) {
        const defaultRules = [
            {
    ...
    static async getStatistics() {
        ...
        const rulesByMetricType = {
            'api_response_time': 0,
            'cpu_usage': 0,
            'memory_usage': 0,
            'error_rate': 0
        };
        const rulesByNotificationChannel = {
            'email': 0,
            'dashboard': 0,
            'sms': 0
        };
        data?.forEach(rule => {
            rulesByMetricType[rule.metric_type]++;
            rule.notification_channels.forEach(channel => {
                rulesByNotificationChannel[channel]++;
            });
        });
        return {
            totalRules,
     ...
     */
    static async bulkUpdateStatus(ruleIds, isActive) {
        const { data, error } = await supabase_1.supabase
            .from('alert_rules')
            .update({
            is_active: isActive,
     ...
     */
    static async getRecentlyTriggered(hours = 24) {
...
exports.AlertRuleModel = AlertRuleModel;
