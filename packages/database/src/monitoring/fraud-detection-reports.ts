...
export class FraudDetectionReportModel {
  /**
   * Create or update fraud detection report for a store and date
   */
  static async upsertReport(reportData: FraudReportData): Promise<FraudDetectionReport | null> {
  ...
  static async getReports(
    ...
    page = 1,
    limit = 50
  ): Promise<{ reports: FraudDetectionReport[]; total: number }> {
    let query = supabase
      .from('fraud_detection_reports')
      .select('*', { count: 'exact' });
  ...
  static async getByStoreAndDate(
    storeId: string,
    reportDate: string
  ): Promise<FraudDetectionReport | null> {
   ...
   */
  static async getStoreFraudTrends(
    storeId: string,
    days = 30
  ): Promise<{
    trends: {
   ...
   */
  static async getGlobalFraudStats(days = 30): Promise<{
    totalReports: number;
    avgFailureRate: number;
    totalBlockedTransactions: number;
    storesWithHighFraud: number; // >10% failure rate
    ...
    const storesWithHighFraud = new Set(
      data?.filter(r => r.verification_failure_rate > 10)
        .map(r => r.store_id)
    ).size;

    // Aggregate suspicious patterns
    const patternStats: { [patternType: string]: { count: number; stores: Set<string> } } = {};
    ...
    data?.forEach(report => {
      if (report.suspicious_patterns) {
        Object.keys(report.suspicious_patterns).forEach(patternType => {
          ...
          patternStats[patternType].count += (report.suspicious_patterns as any)[patternType] || 0;
   ...
   */
  static async getHighRiskStores(
    ...
    const storeMetrics: { [storeId: string]: {
      failureRates: number[];
      totalBlocked: number;
      lastReportDate: string;
    } } = {};
   ...
   */
  static async updateSuspiciousPatterns(
    storeId: string,
    reportDate: string,
   ...
   */
  static async updateAccuracyMetrics(
    storeId: string,
    reportDate: string,
   ...
   */
  static async getPerformanceMetrics(days = 30): Promise<{
    ...
    data?.forEach(report => {
      const metrics = report.accuracy_metrics as AccuracyMetric;
      if (metrics) {
        ...
        if (metrics.precision !== undefined) totalPrecision += metrics.precision;
        if (metrics.recall !== undefined) totalRecall += metrics.recall;
        if (metrics.f1Score !== undefined) totalF1 += metrics.f1Score;
        validReports++;
      }
    });
    ...
    let modelPerformance: 'excellent' | 'good' | 'fair' | 'poor' = 'poor';
    if (avgF1Score >= 0.9) {
   ...
   */
  static async generateAlertSummary(): Promise<{
