...
exports.RevenueAnalyticsModel = void 0;
...
class RevenueAnalyticsModel {
     ...
     */
    static async upsertRevenue(revenueData) {
     ...
     */
    static async getRevenueAnalytics(filters, page = 1, limit = 50) {
     ...
     */
    static async getStoreRevenueTrends(storeId, days = 30) {
     ...
     */
    static async getAggregatedStats(storeIds, days = 30) {
        const endDate = new Date().toISOString().split('T')[0];
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const startDateStr = startDate.toISOString().split('T')[0];
        ...
        const rewardDistributionSummary = {
            grade_a: 0,
            grade_b: 0,
            grade_c: 0,
            grade_d: 0
        ...
        data?.forEach(record => {
            if (record.reward_distribution) {
                const distribution = record.reward_distribution;
     ...
     */
    static async getRevenueComparison(storeId, currentPeriodDays = 30, previousPeriodDays = 30) {
        ...
        // Build queries
        let currentQuery = supabase_1.supabase
            .from('revenue_analytics')
        ...
        let previousQuery = supabase_1.supabase
            .from('revenue_analytics')
            .select('net_revenue, total_rewards_paid, feedback_volume')
     ...
     */
    static async getRewardDistributionAnalysis(storeId, days = 30) {
        const endDate = new Date().toISOString().split('T')[0];
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);
        const startDateStr = startDate.toISOString().split('T')[0];
     ...
     */
    static async getProfitabilityAnalysis(storeId, days = 30) {
     ...
     */
    static async generateRevenueInsights(days = 30) {
...
exports.RevenueAnalyticsModel = RevenueAnalyticsModel;
