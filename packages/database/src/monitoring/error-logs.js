...
class ErrorLogModel {
     ...
     */
    static async getErrorLogs(filters, page = 1, limit = 50) {
     ...
     */
    static async updateResolutionStatus(id, status) {
        const { data, error } = await supabase_1.supabase
            .from('error_logs')
            .update({
     ...
     */
    static async getCriticalErrors(hours = 24) {
     ...
     */
    static async getUnresolvedErrorStats() {
        const { data, error } = await supabase_1.supabase
            .from('error_logs')
            .select('severity')
     ...
     */
    static async getErrorTrendsByService(days = 7) {
        const since = new Date();
     ...
     */
    static async getFrequentErrorPatterns(limit = 10, hours = 24) {
        const since = new Date();
        since.setHours(since.getHours() - hours);
        const { data, error } = await supabase_1.supabase
            .from('error_logs')
     ...
     */
    static async getErrorsByEndpoint(endpoint, hours = 24) {
        const since = new Date();
        since.setHours(since.getHours() - hours);
        const { data, error } = await supabase_1.supabase
            .from('error_logs')
     ...
     */
    static async logCritical(serviceName, errorMessage, stackTrace, endpoint, userContext) {
     ...
     */
    static async logWarning(serviceName, errorMessage, endpoint, userContext) {
        return this.logError({
            severity: 'warning',
            errorMessage,
            serviceName,
     ...
     */
    static async logInfo(serviceName, errorMessage, endpoint, userContext) {
        return this.logError({
            severity: 'info',
            errorMessage,
            serviceName,
     ...
     */
    static async markAsInvestigating(id) {
     ...
     */
    static async getErrorRate(serviceName, hours = 24) {
        const since = new Date();
        since.setHours(since.getHours() - hours);
        let query = supabase_1.supabase
            .from('error_logs')
