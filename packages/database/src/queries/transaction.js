...
exports.createTransactionQueries = createTransactionQueries;
...
class TransactionQueries {
    ...
    async create(data, authContext) {
        try {
            utils_js_1.dbLogger.debug('Creating transaction', { store_id: data.store_id });
            ...
            const { data: transaction, error } = await this.client
                ...
                .single();
            if (error) {
                utils_js_1.dbLogger.error('Failed to create transaction', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            utils_js_1.dbLogger.info('Transaction created successfully', { id: transaction.id, store_id: transaction.store_id });
            return transaction;
        }
        catch (error) {
    ...
    async createFromToleranceInput(storeId, toleranceInput, authContext) {
        try {
    ...
    async findById(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding transaction by ID', { id });
            ...
            if (error) {
                if (error.code === 'PGRST116') {
                    utils_js_1.dbLogger.debug('Transaction not found', { id });
                    return null;
                }
                utils_js_1.dbLogger.error('Failed to find transaction by ID', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return transaction;
        }
    ...
    async findByStoreId(storeId, filters = {}, pagination = { page: 1, limit: 50 }, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding transactions by store ID', { storeId });
            await this.validateStoreAccess(storeId, authContext);
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to find transactions by store ID', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
    ...
    async findWithFeedback(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding transaction with feedback', { id });
            const query = this.client
                .from('transactions')
                .select(`
          *,
        ...
        `)
            ...
            if (error) {
                ...
                utils_js_1.dbLogger.error('Failed to find transaction with feedback', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return transaction;
        }
    ...
    async findPendingVerification(storeId, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding pending verification transactions', { storeId });
            let query = this.client
                .from('transactions')
                .select('*')
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to find pending verification transactions', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return transactions || [];
        }
    ...
    async update(id, data, authContext) {
        try {
            utils_js_1.dbLogger.debug('Updating transaction', { id, fields: Object.keys(data) });
            const query = this.client
                .from('transactions')
                .update(data)
                .eq('id', id);
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to update transaction', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            utils_js_1.dbLogger.info('Transaction updated successfully', { id });
            return transaction;
        }
        catch (error) {
            if (error instanceof Error && 'code' in error) {
    ...
    async verifyTransaction(id, actualAmount, actualTime, authContext) {
        try {
            utils_js_1.dbLogger.debug('Verifying transaction', { id, actualAmount, actualTime });
            ...
            utils_js_1.dbLogger.info('Transaction verification completed', { id, is_match: result.is_match });
            return result;
        }
        catch (error) {
            if (error instanceof Error && 'code' in error) {
    ...
    async findMatchingTransactions(storeId, actualAmount, actualTime, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding matching transactions', { storeId, actualAmount, actualTime });
            await this.validateStoreAccess(storeId, authContext);
    ...
    async delete(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Deleting transaction', { id });
            const query = this.client
                .from('transactions')
                .delete()
                .eq('id', id);
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to delete transaction', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            utils_js_1.dbLogger.info('Transaction deleted successfully', { id });
        }
        catch (error) {
            if (error instanceof Error && 'code' in error) {
                throw error;
    ...
    async count(storeId, verificationStatus, authContext) {
        try {
            let query = this.client
                .from('transactions')
    ...
    async createTimeToleranceRange(timeRangeString) {
        try {
            ...
            if (error) {
                utils_js_1.dbLogger.warn('Failed to create time tolerance range, using fallback', error);
                return this.createFallbackTimeRange(timeRangeString);
            }
            return data;
        }
        catch (error) {
            utils_js_1.dbLogger.warn('Error creating time tolerance range, using fallback', error);
            return this.createFallbackTimeRange(timeRangeString);
        }
    }
    async createAmountToleranceRange(amountRangeString) {
        try {
            ...
            if (error) {
                utils_js_1.dbLogger.warn('Failed to create amount tolerance range, using fallback', error);
                return this.createFallbackAmountRange(amount);
            }
            return data;
        }
        catch (error) {
            utils_js_1.dbLogger.warn('Error creating amount tolerance range, using fallback', error);
            const amount = parseFloat(amountRangeString) || 0;
            return this.createFallbackAmountRange(amount);
        }
    }
    async createTimeToleranceFromTimestamp(timestamp) {
        return await this.createTimeToleranceRange(timestamp);
    }
    async createAmountToleranceFromValue(amount) {
        return await this.createAmountToleranceRange(amount.toString());
    }
    createFallbackTimeRange(timeString) {
    ...
    async checkToleranceMatch(transaction, actualAmount, actualTime) {
        ...
        const timeDifferenceMs = Math.min(Math.abs(actualDateTime.getTime() - timeRange.start.getTime()), Math.abs(actualDateTime.getTime() - timeRange.end.getTime()));
        const timeDifferenceMinutes = timeDifferenceMs / (1000 * 60);
        const amountDifference = Math.min(Math.abs(actualAmount - amountRange.min), Math.abs(actualAmount - amountRange.max));
    ...
    parseTimeRange(timeRangeString) {
    ...
    parseAmountRange(amountRangeString) {
...
function createTransactionQueries(client) {
