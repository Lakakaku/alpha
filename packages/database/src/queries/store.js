...
exports.createStoreQueries = createStoreQueries;
...
class StoreQueries {
    ...
    async create(data, authContext) {
        try {
            utils_js_1.dbLogger.debug('Creating store', { name: data.name, business_id: data.business_id });
            ...
            const { data: store, error } = await this.client
                ...
                .single();
            if (error) {
                utils_js_1.dbLogger.error('Failed to create store', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            utils_js_1.dbLogger.info('Store created successfully', { id: store.id, name: store.name });
            return store;
        }
        catch (error) {
    ...
    async findById(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding store by ID', { id });
            ...
            if (error) {
                if (error.code === 'PGRST116') {
                    utils_js_1.dbLogger.debug('Store not found', { id });
                    return null;
                }
                utils_js_1.dbLogger.error('Failed to find store by ID', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return store;
        }
    ...
    async findByQRCode(qrCodeData, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding store by QR code', { qrCodeData: qrCodeData.substring(0, 20) + '...' });
            const query = this.client
                .from('stores')
                .select('*')
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to find store by QR code', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return store;
        }
    ...
    async findByBusinessId(businessId, filters = {}, pagination = { page: 1, limit: 50 }, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding stores by business ID', { businessId });
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to find stores by business ID', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
    ...
    async findWithContext(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding store with context', { id });
            const query = this.client
                .from('stores')
                .select(`
          *,
        ...
        `)
            ...
            if (error) {
                ...
                utils_js_1.dbLogger.error('Failed to find store with context', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return store;
        }
    ...
    async findWithFeedback(id, dateRange, authContext) {
        try {
            utils_js_1.dbLogger.debug('Finding store with feedback', { id, dateRange });
            const query = this.client
                .from('stores')
                .select(`
          *,
        ...
        `)
            ...
            if (error) {
                ...
                utils_js_1.dbLogger.error('Failed to find store with feedback', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return store;
        }
    ...
    async update(id, data, authContext) {
        try {
            utils_js_1.dbLogger.debug('Updating store', { id, fields: Object.keys(data) });
            const query = this.client
                .from('stores')
                .update(data)
                .eq('id', id);
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to update store', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            utils_js_1.dbLogger.info('Store updated successfully', { id, name: store.name });
            return store;
        }
        catch (error) {
            if (error instanceof Error && 'code' in error) {
    ...
    async delete(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Deleting store', { id });
            const query = this.client
                .from('stores')
                .delete()
                .eq('id', id);
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to delete store', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            utils_js_1.dbLogger.info('Store deleted successfully', { id });
        }
        catch (error) {
            if (error instanceof Error && 'code' in error) {
                throw error;
    ...
    async setActive(id, isActive, authContext) {
        try {
            utils_js_1.dbLogger.debug('Setting store active status', { id, isActive });
    ...
    async regenerateQRCode(id, authContext) {
        try {
            utils_js_1.dbLogger.debug('Regenerating QR code for store', { id });
    ...
    async updateProfile(id, profileUpdates, authContext) {
        try {
            const store = await this.findById(id, authContext);
            if (!store) {
                throw new Error('Store not found');
    ...
    async count(businessId, authContext) {
        try {
            let query = this.client
                .from('stores')
    ...
    generateQRCodeData(businessId, storeName) {
        const timestamp = Date.now();
        const random = Math.random().toString(36).substring(2, 15);
        const storeSlug = storeName.toLowerCase().replace(/[^a-z0-9]/g, '-');
        return `${businessId}-${storeSlug}-${timestamp}-${random}`;
    }
    async searchByLocation(coordinates, radiusKm = 10, authContext) {
        try {
            utils_js_1.dbLogger.debug('Searching stores by location', { coordinates, radiusKm });
            let query = this.client
                .from('stores')
                .select('*')
                .eq('is_active', true);
            ...
            if (error) {
                utils_js_1.dbLogger.error('Failed to search stores by location', error);
                throw (0, utils_js_1.formatDatabaseError)(error);
            }
            return stores || [];
        }
...
function createStoreQueries(client) {
