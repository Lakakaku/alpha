...
import type {
  Database,
  VerificationRecord,
  VerificationRecordInsert,
  VerificationRecordUpdate,
  VerificationRecordWithBusiness,
  WeeklyVerificationStatus,
  WeeklyVerificationData,
  WeeklyVerificationSubmission,
  PaginationParams,
  PaginatedResponse,
  AuthContext
} from '../types/index.js';
...
export class VerificationRecordQueries {
  ...
  async create(data: VerificationRecordInsert, authContext?: AuthContext): Promise<VerificationRecord> {
    try {
      dbLogger.debug('Creating verification record', {
      ...
      if (error) {
        dbLogger.error('Failed to create verification record', error);
        throw formatDatabaseError(error);
      }

      dbLogger.info('Verification record created successfully', {
  ...
  async findById(id: string, authContext?: AuthContext): Promise<VerificationRecord | null> {
    try {
      dbLogger.debug('Finding verification record by ID', { id });
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          dbLogger.debug('Verification record not found', { id });
          return null;
        }
        dbLogger.error('Failed to find verification record by ID', error);
        throw formatDatabaseError(error);
      }
  ...
  async findByBusinessAndWeek(
    businessId: string,
    weekIdentifier: string,
    authContext?: AuthContext
  ): Promise<VerificationRecord | null> {
    try {
      dbLogger.debug('Finding verification record by business and week', { businessId, weekIdentifier });
      ...
      if (error) {
        dbLogger.error('Failed to find verification record by business and week', error);
        throw formatDatabaseError(error);
      }
  ...
  async findByBusinessId(
    ...
    try {
      dbLogger.debug('Finding verification records by business ID', { businessId });
      ...
      if (error) {
        dbLogger.error('Failed to find verification records by business ID', error);
        throw formatDatabaseError(error);
      }
  ...
  async findByStatus(
    ...
    try {
      dbLogger.debug('Finding verification records by status', { status });
      ...
      if (error) {
        dbLogger.error('Failed to find verification records by status', error);
        throw formatDatabaseError(error);
      }
  ...
  async findWithBusiness(id: string, authContext?: AuthContext): Promise<VerificationRecordWithBusiness | null> {
    try {
      dbLogger.debug('Finding verification record with business', { id });
      ...
      if (error) {
        ...
        dbLogger.error('Failed to find verification record with business', error);
        throw formatDatabaseError(error);
      }

      return verificationRecord as VerificationRecordWithBusiness;
    } catch (error) {
      if (error instanceof Error && 'code' in error) {
        throw error;
      }
  ...
  async update(id: string, data: VerificationRecordUpdate, authContext?: AuthContext): Promise<VerificationRecord> {
    try {
      dbLogger.debug('Updating verification record', { id, fields: Object.keys(data) });
      ...
      if (error) {
        dbLogger.error('Failed to update verification record', error);
        throw formatDatabaseError(error);
      }

      dbLogger.info('Verification record updated successfully', {
        id,
        status: verificationRecord.status,
        week_identifier: verificationRecord.week_identifier
      });
  ...
  async submitForVerification(
    id: string,
    submissionData: WeeklyVerificationSubmission,
    authContext?: AuthContext
  ): Promise<VerificationRecord> {
    try {
      dbLogger.debug('Submitting verification record', { id });
      ...
      const updateData: VerificationRecordUpdate = {
        status: 'submitted',
  ...
  async completeVerification(
    ...
    try {
      dbLogger.debug('Completing verification record', { id });
  ...
  async generateWeeklyVerificationData(
    businessId: string,
    weekIdentifier: string,
    authContext?: AuthContext
  ): Promise<WeeklyVerificationData> {
    try {
      dbLogger.debug('Generating weekly verification data', { businessId, weekIdentifier });
      ...
      if (transactionsError) {
        throw formatDatabaseError(transactionsError);
      }
      ...
      if (feedbackError) {
        throw formatDatabaseError(feedbackError);
      }

      const verificationData: WeeklyVerificationData = {
  ...
  async createWeeklyRecord(
    ...
    try {
      ...
      const recordData: VerificationRecordInsert = {
        business_id: businessId,
        week_identifier: weekIdentifier,
        status: 'pending',
        transaction_summary: {
  ...
  async delete(id: string, authContext?: AuthContext): Promise<void> {
    try {
      dbLogger.debug('Deleting verification record', { id });
      ...
      if (error) {
        dbLogger.error('Failed to delete verification record', error);
        throw formatDatabaseError(error);
      }

      dbLogger.info('Verification record deleted successfully', { id });
    } catch (error) {
      if (error instanceof Error && 'code' in error) {
        throw error;
      }
  ...
  async exists(businessId: string, weekIdentifier: string, authContext?: AuthContext): Promise<boolean> {
    try {
  ...
  async getNextWeekIdentifier(currentWeek: string): Promise<string> {
  ...
  async getPreviousWeekIdentifier(currentWeek: string): Promise<string> {
    const [year, week] = currentWeek.split('-W').map(Number);
  ...
  private isValidWeekIdentifier(weekIdentifier: string): boolean {
    const weekPattern = /^\d{4}-W(0[1-9]|[1-4]\d|5[0-2])$/;
    return weekPattern.test(weekIdentifier);
  }

  private parseWeekIdentifier(weekIdentifier: string): { start: string; end: string } {
  ...
  async findPendingSubmissions(authContext?: AuthContext): Promise<VerificationRecord[]> {
    try {
  ...
  async findSubmittedRecords(authContext?: AuthContext): Promise<VerificationRecord[]> {
    try {
  ...
  async getCurrentWeekIdentifier(): Promise<string> {
    const now = new Date();
...
export function createVerificationRecordQueries(client: SupabaseClient<Database>): VerificationRecordQueries {
