...
exports.shouldRetryOperation = shouldRetryOperation;
...
async function checkTablesAccessible(client) {
...
async function checkFunctionsAvailable(client) {
...
function shouldRetryOperation(error) {
...
async function retryWithExponentialBackoff(operation, maxRetries = 3, baseDelayMs = 1000) {
    let lastError;
...
function createDatabaseLogger(prefix = '[Database]') {
    return {
        info: (message, data) => {
...
exports.dbLogger = createDatabaseLogger();
// Type guards
