...
export async function GET(request: NextRequest) {
  try {
    ...
    const currentStore: Store = {
      ...
      permissions: {
        readFeedback: currentSession.business_stores.permissions?.read_feedback || false,
        writeContext: currentSession.business_stores.permissions?.write_context || false,
        manageQr: currentSession.business_stores.permissions?.manage_qr || false,
        viewAnalytics: currentSession.business_stores.permissions?.view_analytics || false,
  ...
  return NextResponse.json(
    {
      ...
      message: 'POST method not supported for this endpoint. Use PUT to update store context'
    } as ErrorResponse,
    { status: 405 }
  );
}
